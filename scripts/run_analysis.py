...
# ── engine imports ───────────────────────────────────────────────────────────
from engine.trading.bybit_client import fetch_last_klines, BybitPrivateClient
...
from engine.utils.helpers import taker_fee_for, fetch_borrow_rate_safe
import pandas as pd
import numpy as np
...
def analyse_interval(symbol: str, interval: str, days: int = 1, trials: int = 4000):
    ...
    # Fetch live borrow rate; use constant default on failure
    _borrow_rate = fetch_borrow_rate_safe(BybitPrivateClient())
...
def run_analysis(symbol: str, days: int = 1, trials: int = 4000):
    ...
    if valid:
        ...
        print(f"     Trades={best['trades']}  WR={best['win_rate']:.1f}%  "
              f"PnL={best['pnl_pct']:+.2f}%  MaxDD={best['max_dd']:.1f}%")
