...
def fetch_borrow_rate_safe(client: Any) -> float:
    """Fetch the live spot-margin USDT hourly borrow rate from Bybit.
    Returns BORROW_HOURLY_RATE constant on any failure."""
    try:
        return client.fetch_spot_margin_borrow_rate(coin="USDT")
    except Exception:
