...
def fetch_mark_klines(symbol: str, interval: str, start_ms: int, end_ms: int) -> pd.DataFrame:
    """Spot margin has no mark price — return last-price klines."""
    return fetch_last_klines(symbol, interval, start_ms, end_ms)

def fetch_risk_tiers(symbol: str) -> pd.DataFrame:
    """Spot margin has no risk tiers — returns empty DataFrame."""
...
class BybitPrivateClient:
    ...
    Designed for Unified Trading Account (UTA) spot margin LONG-only trading."""
    ...
    def _toggle_spot_margin_mode(self, enable: bool = True) -> None:
        """Enable or disable spot margin trading on the UTA account.
        Must be called before placing any order with isLeverage=1.
        Uses the dedicated spot-margin-trade endpoint (NOT the futures endpoint).
        Silently skips if the account doesn't support spot margin or mode is already set."""
        mode = "1" if enable else "0"
        try:
            j = rest_request(
                "POST",
                "/v5/spot-margin-trade/switch-mode",
                body={"spotMarginMode": mode},
                auth=True
            )
            result_mode = j.get("result", {}).get("spotMarginMode", mode)
            log.info("Spot margin mode set to %s (response: %s)", mode, result_mode)
        except RuntimeError as exc:
            msg = str(exc)
            # 10001 = mode already set; ignore silently
            if "10001" in msg or "already" in msg.lower():
                log.info("Spot margin mode already at %s for this account.", mode)
                return
            log.warning("_toggle_spot_margin_mode failed (trading may still work): %s", exc)

    def set_leverage(self, symbol: str, buy_leverage: float, sell_leverage: float = 0.0):
        """Set spot margin leverage via /v5/spot-margin-trade/set-leverage.
        Silently skips if leverage is already at the requested value."""
        try:
            rest_request(
                "POST",
                "/v5/spot-margin-trade/set-leverage",
                body={"leverage": str(int(buy_leverage))},
                auth=True
            )
        except RuntimeError as exc:
            msg = str(exc)
    ...
    def ensure_trading_setup(self, symbol: str, leverage: float = 1.0):
        """Enable spot margin mode and set leverage before live trading."""
        lev = leverage if leverage > 1.0 else leverage_for(symbol)
        self._toggle_spot_margin_mode(enable=True)
        self.set_leverage(symbol, lev, lev)

    def fetch_spot_margin_borrow_rate(self, coin: str = "USDT") -> float:
        """Fetch the current hourly borrowing rate for a coin on Bybit spot margin.
        Uses /v5/spot-margin-trade/data to get the daily interest rate, then converts
        to hourly by dividing by 24.  Falls back to BORROW_HOURLY_RATE constant on error.

        Args:
            coin: The coin being borrowed — USDT for LONG spot margin.
        Returns:
            Hourly borrow rate as a decimal fraction (e.g. 0.0001 = 0.01%/hr).
        """
        try:
            j = rest_request(
                "GET",
            ...
            for row in rows:
                if row.get("coin", "").upper() == coin.upper():
                    # Bybit returns hourlyBorrowRate or dailyBorrowRate as a string
                    hourly = row.get("hourlyBorrowRate") or row.get("dailyBorrowRate")
                    if hourly is not None:
                        rate = float(hourly)
                        # If it looks like a daily rate (significantly larger than typical hourly)
                        if rate > 0.001:
                            rate = rate / 24.0
                        log.info("Spot margin hourly borrow rate for %s: %.6f%%", coin, rate * 100)
                        return rate
        except Exception as exc:
            log.warning("fetch_spot_margin_borrow_rate(%s) failed — using default: %s", coin, exc)
        return BORROW_HOURLY_RATE
    ...
    def get_position(self, symbol: str) -> Optional[RealPosition]:
        """Spot positions are tracked locally by the bot — always returns None."""
        return None
    ...
    def set_trading_stop(self, symbol: str, take_profit: float) -> None:
        """Not supported for spot margin — TP is managed client-side."""
        return
