...
def backtest_once(
    ...
    Args:
        ...
        fee_rate:            Taker fee rate (entries AND exits — both use market/IOC orders)
        ...
        interval_minutes_bt: Duration of each candle in minutes — used to convert
                             hold-candle-count into hours for the time TP and
                             borrow interest calculations.
        borrow_hourly_rate:  Spot margin hourly borrowing rate on the borrowed USDT.
                             Deducted from PnL at exit: borrowed_usdt × rate × hold_hours.
                             borrowed_usdt = notional × (leverage-1)/leverage.
                             Defaults to BORROW_HOURLY_RATE constant (~0.01%/hr).
    ...
    entry_price_bt       = 0.0
    entry_fee            = 0.0
    entry_notional_bt    = 0.0   # notional at entry (for borrow interest calc)
    wallet_at_entry      = 0.0
    in_position          = False
    ...
    # Precompute borrow interest: borrowed_usdt × rate × hold_hours
    # borrowed_usdt = entry_notional × (lev-1)/lev
    _borrow_disabled = borrow_hourly_rate <= 0 or leverage_bt <= 1.0
    _borrow_borrowed_frac = (leverage_bt - 1.0) / leverage_bt  # fraction of notional that is borrowed
    _borrow_per_candle    = _borrow_borrowed_frac * borrow_hourly_rate * interval_minutes_bt / 60.0

    def _borrow_interest(hold_candles_n: int) -> float:
        if _borrow_disabled:
            return 0.0
        return entry_notional_bt * _borrow_per_candle * hold_candles_n
    ...
    for i in range(1, len(dfl)):
        ...
        if in_position and pos_qty != 0.0:
            ...
            if _liq_price_bt > 0 and low_last <= _liq_price_bt:
                ...
                interest  = _borrow_interest(i - entry_candle_idx)
                wallet    = max(0.0, wallet + pnl_gross - exit_fee - interest)
                pnl_net   = pnl_gross - entry_fee - exit_fee - interest
                ...
                _liq_price_bt = 0.0; _max_high_bt = 0.0; entry_notional_bt = 0.0
            ...
            if not exited and pos_qty != 0.0:
                ...
                if high_last >= tp_price:
                    ...
                    exit_fee  = (qty_abs * fill) * fee_rate
                    interest  = _borrow_interest(i - entry_candle_idx)
                    wallet   += pnl_gross - exit_fee - interest
                    pnl_net   = pnl_gross - entry_fee - exit_fee - interest
                    trade_pnls.append(pnl_net)
                    _exit_ts_raw = dfl.iloc[i]["ts"]
                    _exit_ts_ms  = int(_exit_ts_raw.value) // 1_000_000 if hasattr(_exit_ts_raw, "value") else int(_exit_ts_raw)
        ...
        if not exited and not in_position and wallet > 0.0:
            ...
            if resolve_entry_signals(
                ...
                if _candle_usdt_vol > 0 and (_pos_notional / _candle_usdt_vol) > VOL_FILTER_MAX_PCT:
                    ...
                    qty             = notional / fill
                    # Entry uses market/IOC order in live → taker fee applies
                    fee             = notional * fee_rate
                    entry_notional_bt = notional
                    wallet         -= fee
