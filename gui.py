...
class _BotController:
    ...
    def _run(self) -> None:
        try:
            ...
            # Fetch live spot margin borrow rate; fall back to constant default
            _borrow_rate = C.BORROW_HOURLY_RATE
            try:
                _borrow_rate = client.fetch_spot_margin_borrow_rate(coin="USDT")
                self._log(f"Borrow rate (USDT): {_borrow_rate*100:.4f}%/hr")
            except Exception as _br_err:
                self._log(f"Borrow rate fetch failed — using default {_borrow_rate*100:.4f}%/hr")
            ...
            for sym in symbols:
                for iv in intervals:
                    ...
                    self._emit("agent",
                               ...
                               f"Downloading {iv}m…")
                    df_last = bot.download_seed_history(sym, C.DAYS_BACK_SEED, iv)
