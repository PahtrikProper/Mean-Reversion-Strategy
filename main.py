...
from engine.utils.helpers import (
    leverage_for, taker_fee_for, supported_intervals,
)
...
def run_live_trading():
    ...
    # ── Fetch live spot margin borrow rate for USDT ───────────────────────────
    _borrow_rate = const_module.BORROW_HOURLY_RATE
    try:
        _borrow_rate = client.fetch_spot_margin_borrow_rate(coin="USDT")
        print(f"  Spot margin hourly borrow rate (USDT): {_borrow_rate*100:.4f}%/hr")
    except Exception as _borrow_err:
        print(f"  Borrow rate fetch failed — using default {_borrow_rate*100:.4f}%/hr: {_borrow_err}")
    ...
    for symbol in symbols:
        for interval in intervals:
            ...
            df_last = bot_module.download_seed_history(
                symbol, const_module.DAYS_BACK_SEED, interval
            )
            print(f"    candles : {len(df_last)}")
            ...
            opt = bot_module.optimise_bayesian(
                ...
                borrow_hourly_rate= _borrow_rate,
